...
const AddDishesSection = ({
    ...
    const filteredDishes = dishCategory ? dishes.filter(d => d.category === dishCategory) : [];
    ...
    return (
        <div className='rounded-2xl border border-[#2f2f2f] bg-[#262626] p-5 lg:col-span-2'>
            ...
            <div className='mt-4 max-h-64 overflow-y-auto overflow-x-auto no-scrollbar rounded-lg border border-[#343434]'>
                <table className='min-w-full text-sm'>
                    <thead className='bg-[#1f1f1f] text-left text-[#cfcfcf] sticky top-0'>
                        <tr>
                    ...
                    <tbody className='bg-[#262626]'>
                        {!dishCategory ? (
                            <tr>
                                <td colSpan={4} className='px-3 py-4 text-center text-[#a8a8a8]'>Please select a category to view dishes.</td>
                            </tr>
                        ) : filteredDishes.length === 0 ? (
                            <tr>
                                <td colSpan={4} className='px-3 py-4 text-center text-[#a8a8a8]'>No dishes added to this category yet.</td>
                            </tr>
                        ) : (
